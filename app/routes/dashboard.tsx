...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  // Don't redirect if accessing a specific family or other dashboard routes
  const url = new URL(request.url);
  if (url.pathname.includes('/family/') || url.pathname.includes('/onboarding') || url.pathname !== '/dashboard') {
    return { user };
  }

  // Check if user has any families (only for main dashboard)
  const userFamilies = await caller.families.getMyFamilies();
        ...
        <div className="flex flex-1 flex-col p-6 bg-gradient-to-b from-orange-50 to-rose-50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-md hover:shadow-lg transition-shadow border-l-4 border-indigo-500">
              ...
              <button className="mt-4 px-4 py-2 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white rounded-lg font-medium hover:from-indigo-600 hover:to-indigo-700 transition-all" disabled>
                My Files (Coming Soon)
              </button>
            </div>
          </div>
